...
const api = {
    ...
    // ─── Admin ───────────────────────────────────────────
    fetchAdminUsers() { return fetchWithAuth('/api/admin/users', { headers: authHeaders() }).then(async r => { if (!r.ok) throw new Error((await r.json()).error || 'Failed'); return r.json(); }); },
    ...
    bulkDeleteUsers(ids) { return fetchWithAuth('/api/admin/users/bulk-delete', { method: 'POST', headers: { ...authHeaders(), 'Content-Type': 'application/json' }, body: JSON.stringify({ ids }) }).then(async r => { if (!r.ok) throw new Error((await r.json()).error || 'Failed'); return r.json(); }); },
    fetchAdminOrders() { return fetchWithAuth('/api/admin/orders', { headers: authHeaders() }).then(async r => { if (!r.ok) throw new Error((await r.json()).error || 'Failed'); return r.json(); }); },
    updateAdminOrderStatus(id, status) { return apiFetchWithBody('/api/admin/orders/' + id + '/status', 'PUT', { status }); },
    deleteAdminOrder(id) { return fetchWithAuth('/api/admin/orders/' + id, { method: 'DELETE', headers: authHeaders() }).then(async r => { if (!r.ok) throw new Error((await r.json()).error || 'Failed'); return r.json(); }); },
    fetchAdminSellers() { return fetchWithAuth('/api/admin/sellers', { headers: authHeaders() }).then(async r => { if (!r.ok) throw new Error((await r.json()).error || 'Failed'); return r.json(); }); },
    fetchAdminBuyers() { return fetchWithAuth('/api/admin/buyers', { headers: authHeaders() }).then(async r => { if (!r.ok) throw new Error((await r.json()).error || 'Failed'); return r.json(); }); },
    fetchAdminAnalytics() { return fetchWithAuth('/api/admin/analytics', { headers: authHeaders() }).then(async r => { if (!r.ok) throw new Error((await r.json()).error || 'Failed'); return r.json(); }); },
    fetchAdminAnnouncements() { return fetchWithAuth('/api/admin/announcements', { headers: authHeaders() }).then(async r => { if (!r.ok) throw new Error((await r.json()).error || 'Failed'); return r.json(); }); },
