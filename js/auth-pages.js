// js/auth-pages.js
// Shared auth page utilities — validation, password strength, loading states
...
function renderPasswordStrength(password, containerId) {
    ...
    const colors = ['', 'var(--error-color)', '#f59e0b', '#f59e0b', 'var(--accent-green)', 'var(--accent-green)'];
    const widths = ['', '20%', '40%', '60%', '80%', '100%'];

    container.innerHTML = `
        <div style="display:flex;gap:3px;margin-bottom:0.35rem;">
            ${[1,2,3,4,5].map(i => `<div style="flex:1;height:4px;border-radius:2px;background:${i <= score ? colors[score] : 'var(--border-color)'};transition:background 0.3s;"></div>`).join('')}
        </div>
